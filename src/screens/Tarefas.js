...
import { StyleSheet, Text, View, FlatList, ActivityIndicator, Platform, SafeAreaView,TouchableOpacity,Image,TextInput, Alert} from 'react-native';
...
export function TarefasScreen({navigation}) {
  ...
  const API_URL = `http://localhost:3000/tarefas/${item.idTarefa}`;
  ...
  const handleDelete = async (id) => {
    const API_URL = `http://localhost:3000/tarefas/${id}`;
    try {
       if (Platform.OS === 'web') {
  ...
  const API_URL = `http://localhost:3000/tarefas/usuario/${idUsuario}`;

  try {
    const response = await axios.get(API_URL);

    setTarefas(response.data.saved);
  } catch (error) {
    setTarefas([]);
        ...
        <View style={styles.cardButtonContainer}>
          <TouchableOpacity onPress={() => handleBeginUpdate(item)}
            style={styles.editBtn}>
              <Text style={styles.editBtnImg}>Editar</Text>
          </TouchableOpacity>

          <TouchableOpacity onPress={() => handleDelete(item.idTarefa)}
            style={styles.deleteBtn}>
              <Text style={styles.deleteBtnImg}>Excluir</Text>
          </TouchableOpacity>
        </View>
